...
Latitude and Longitude converted to census tracts and joined with database from the American Neighbourhood Change:
...
Hawaii_gentrification <- Hawaii_gentrification %>% clean_names() %>% filter(state_name == 'Hawaii') %>% rename("GEOID" = "geoid") %>%  select(-'state_name')
...
Hawaii_gentrification <- Hawaii_gentrification %>% select( - 'county_name')
...
coord_2012$GEOID <- substr(coord_2012$GEOID, 0, 11)

```

```{r}
Hawaii_gentrification_2012 <- coord_2012 %>% 
...
coord_2013$GEOID <- substr(coord_2013$GEOID, 0, 11)

```

```{r}
Hawaii_gentrification_2013 <- coord_2013 %>% 
inner_join(Hawaii_gentrification, by=c('GEOID')) 
``` 
...
coord_2014$GEOID <- substr(coord_2014$GEOID, 0, 11)

```


```{r}
Hawaii_gentrification_2014 <- coord_2014 %>% 
inner_join(Hawaii_gentrification, by=c('GEOID')) 
``` 
...
coord_2015$GEOID <- substr(coord_2015$GEOID, 0, 11)

```

```{r}
Hawaii_gentrification_2015 <- Hawaii_gentrification %>% 
inner_join( coord_2015, by=c('GEOID')) 
``` 
...
coord_2016$GEOID <- substr(coord_2016$GEOID, 0, 11)

```

```{r}
Hawaii_gentrification_2016 <- Hawaii_gentrification %>% 
inner_join( coord_2016, by=c('GEOID')) 
``` 
...
coord_2017$GEOID <- substr(coord_2017$GEOID, 0, 11)

```

```{r}
Hawaii_gentrification_2017 <- Hawaii_gentrification %>% 
inner_join( coord_2017, by=c('GEOID')) 
``` 
...
coord_2018$GEOID <- substr(coord_2018$GEOID, 0, 11)

```

```{r}
Hawaii_gentrification_2018 <- Hawaii_gentrification %>% 
inner_join( coord_2018, by=c('GEOID')) 
``` 
...
coord_2019$GEOID <- substr(coord_2019$GEOID, 0, 11)

```

```{r}
Hawaii_gentrification_2019 <- Hawaii_gentrification %>% 
inner_join( coord_2019, by=c('GEOID')) 
``` 

...
This code assumes use of .Rproj for file pathing. If experiencing difficulties, ensure project "homelesness-project-fall2019" is open.
...
* The U.S. Department of Housing and Urban Development takes Point in Time (PIT) counts of the homeless population yearly for many Continuums of Care (CoCs), and bi-yearly for the remainder. [FLAG ADD LINK]
...
* Although data is available as far back as 2007, experts we spoke with agree the data becomes increasingly less reliable the farther back in time one looks. We felt confident using data from 2011 onward.
* With the exception of Arkansas, all CoC mergers are reflected backards in PIT stats. [CONFIRMED Snow and Byrne]
* Arkansas made changes to its CoC without notifying HUD (according to Snow), thereby making its CoC statistics inconsistent in earlier years. With our decision not to import data prior to 2011, only the CoCs AR-507 and AR-512 remained problematic. To avoid confusion, we chose to drop them from our table.
* Early joins of HUD PIT data and Zillow Cluster study data showed the following discrepencies:
  * The following CoCs were **in the HUD PIT file but not in the Zillow file**. Therefore, we therefore chose to remove all except al-505 from our dataset. 
    * al-505: this CoC is the only one that did not coincide with incomplete data on itself. Byrne said there were some problems matching the .shp tracts against the CoCs. That may explain this CoC. [FLAG MAKE DECISION]
    * ar-504, ar-508
    * ca-527, ca-529, ca-530, ca-528: ca-527 was problematic prior to 2010, which is probably why Zillow dropped it.
    * ga-502, ga-508
    * ma-501
    * nj-512
    * ny-506 
  * Early data load in showed the following CoCs had **incomplete data**. There was high overlap between these and CoCs not present in the Zillow dataset, and we therefore chose to remove them from our dataset.
    * ar-504, ar-508 
    ...
    * nj-512
    * ny-506
  * The following CoCs were **in the Zillow file but not in the HUD PIT file**. These CoCs merged into others in 2018. HUD data has retroactively corrected for this change, but the Zillow study was done prior to the merges and therefore includes the now-defunct CoCs. To account for this, we [FLAG DO SOMETHING].
...
# Load and join Point in Time (PIT) homeless counts from HUD 2007-2018
hud_hic_all <- read_xlsx(paste0(load_path, "2007-2018-HIC-Counts-by-CoC.xlsx"), sheet = 1, col_names=TRUE, skip = 1) %>% # 2018
  full_join(read_xlsx(paste0(load_path, "2007-2018-HIC-Counts-by-CoC.xlsx"), sheet = 2, col_names=TRUE, skip = 1), by=c("CoC Number"), suffix = c("_2018", "_2017")) %>% # 2017
  full_join(read_xlsx(paste0(load_path, "2007-2018-HIC-Counts-by-CoC.xlsx"), sheet = 3, col_names=TRUE, skip = 1) %>%
              # Strip out a from mo-604a; the a is a footnote reference, only appears in 2016
              mutate("CoC Number"=replace("CoC Number", "CoC Number"=="mo-604a", "mo-604")), by=c("CoC Number")) %>% # 2016
  full_join(read_xlsx(paste0(load_path, "2007-2018-HIC-Counts-by-CoC.xlsx"), sheet = 4, col_names=TRUE, skip = 1), by=c("CoC Number"), suffix = c("_2016", "_2015")) %>% # 2015
  full_join(read_xlsx(paste0(load_path, "2007-2018-HIC-Counts-by-CoC.xlsx"), sheet = 5, col_names=TRUE, skip = 1), by=c("CoC Number")) %>% # 2014
  full_join(read_xlsx(paste0(load_path, "2007-2018-HIC-Counts-by-CoC.xlsx"), sheet = 6, col_names=TRUE, skip = 1), by=c("CoC Number"), suffix = c("_2014", "_2013")) %>% # 2013
  full_join(read_xlsx(paste0(load_path, "2007-2018-HIC-Counts-by-CoC.xlsx"), sheet = 7, col_names=TRUE, skip = 1), by=c("CoC Number" = "CoC")) %>% # 2012
  full_join(read_xlsx(paste0(load_path, "2007-2018-HIC-Counts-by-CoC.xlsx"), sheet = 8, col_names=TRUE, skip = 1), by=c("CoC Number" = "CoC"), suffix = c("_2012", "_2011")) %>% # 2011
  # full_join(read_xlsx(paste0(load_path, "2007-2018-HIC-Counts-by-CoC.xlsx"), sheet = 9, col_names=TRUE, skip = 1), by=c("CoC Number" = "CoC")) %>% # 2010
  # full_join(read_xlsx(paste0(load_path, "2007-2018-HIC-Counts-by-CoC.xlsx"), sheet = 10, col_names=TRUE, skip = 1), by=c("CoC Number" = "CoC")) %>% # 2009
  # full_join(read_xlsx(paste0(load_path, "2007-2018-HIC-Counts-by-CoC.xlsx"), sheet = 11, col_names=TRUE, skip = 1), by=c("CoC Number" = "CoC")) %>% # 2008
  # full_join(read_xlsx(paste0(load_path, "2007-2018-HIC-Counts-by-CoC.xlsx"), sheet = 12, col_names=TRUE, skip = 1), by=c("CoC Number" = "CoC") %>% # 2007
  # Clean up column names
  clean_names() %>%
  rename_all(~stringr::str_replace_all(., "co_c", "coc")) %>%
  rename(coc_code = coc_number) %>%
...
* Slicing data by Zillow's [computed clusters](memos/papers-studies-etc/memo-inflection-points.html)
...
Once cleaned in the previous steps, the PIT and Zillow data can be joined and output for further analysis. The following table contains all information also contained in the previous two tables, tidied up for use by reporters.
...
hud_zillow_joined <- hud_pit_all %>%
  ...
  inner_join(zillow_cluster, by= c("coc_code", "state_code")) %>%
...
### Crosswalk: CoC to Tract

This crosswalk was created using [these scripts](https://github.com/tomhbyrne/HUD-CoC-Geography-Crosswalk) from Tom Byrne. 

The following CoCs are in the shapefile but not the PIT count:

* ar-504 (91)
  * ar-504 has already been removed from our analysis for reasons explained above.
* ma-510_&_ma-516_shared_jurisdiction (167)
  * ma-510 and ma-516 are present in both, but 167 tract FIPs also associated with them apparently straddle CoCs. Therefore, I recommend not making judgements about ma-510 or ma-516 based on this crosswalk.

The following CoCs are in the PIT count but not the tract shapefile:
...
crosswalk <- read_csv(paste0(load_path, "tract-coc-match.csv")) %>%
  select(1:4, -coc_name_PIT) %>%
  rename(coc_code = coc_number) %>%
  mutate_all(tolower) %>%
  # Get rid of coc_codes that are NA
  filter(!is.na(coc_code)) %>%
  ## Decided to keep the below for mapping to other data
  # # Drop CoCs with incomplete data
  # filter(!(coc_code %in% incomplete_cocs)) %>%
  # # Drop problematic CoCs
  # filter(!(coc_code %in% problematic_cocs)) %>%
  # Drop CoCs in territories
  filter(!str_detect(coc_code, "pr|mp|gu|vi")) %>%
  # Add col to designate state for grouping
  separate(coc_code, sep = "-", into = c("state_code", "coc_number"), remove = F) %>%
...
# Add Zillow clusters to the crosswalk
crosswalk <- crosswalk %>%
  left_join(
    zillow_cluster %>% select(coc_code, zillow_cluster_num = cluster_number_zillow_2017)
  )

#crosswalk %>% write_csv(paste0(here(), "/data/input-data/clean/crosswalk-coc-to-tract.csv"))
...
hud_hic_all %>%
  write_csv(paste0(save_path, "hud-hic-all.csv"))
...
crosswalk %>%
  write_csv(paste0(save_path, "crosswalk-coc-to-tract.csv"))
